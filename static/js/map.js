...
if (data.medical.length > 0) {
    ...
    // 3. EVACUATION & SHELTER SITES (Government Buildings + Schools)
    if (data.evacuation_centers.length > 0) {
        // Separate government buildings and schools
        ...
        html += `
            <details class="facility-details" style="margin-bottom: 0.75rem;">
                <summary style="cursor: pointer; font-weight: 700; color: #059669; padding: 0.875rem; background: linear-gradient(135deg, #d1fae5 0%, #a7f3d0 100%); border-radius: 6px; display: flex; justify-content: space-between; align-items: center; border: 2px solid #059669;">
                    <span>🏛️ Evacuation & Shelter Sites (${data.evacuation_centers.length})</span>
                    <span style="font-size: 0.875rem;">▼</span>
                </summary>
                <div style="margin-top: 0.5rem; padding: 0.75rem; background: #f0fdf4; border-radius: 6px;">
                    <div style="background: #fef3c7; border-left: 3px solid #f59e0b; padding: 0.625rem; border-radius: 4px; margin-bottom: 0.75rem;">
                        <p style="margin: 0; font-size: 0.75rem; color: #92400e; line-height: 1.5;">
                            ℹ️ <strong>Note:</strong> Government buildings and schools commonly serve as evacuation sites during disasters. Total: ${data.evacuation_centers.length} sites (${governmentBuildings.length} government + ${schools.length} schools). Confirm with your local DRRMO for official designations.
                        </p>
                    </div>
        `;
        ...
        if (governmentBuildings.length > 0) {
            html += `<div style="margin-bottom: 0.75rem;"><div style="font-size: 0.75rem; font-weight: 700; color: #166534; text-transform: uppercase; margin-bottom: 0.5rem; padding: 0.5rem; background: white; border-radius: 4px;">🏛️ Government Buildings (${governmentBuildings.length})</div>`;
        ...
        // Schools
        if (schools.length > 0) {
            html += `<div style="margin-bottom: 0;"><div style="font-size: 0.75rem; font-weight: 700; color: #166534; text-transform: uppercase; margin-bottom: 0.5rem; padding: 0.5rem; background: white; border-radius: 4px;">🎓 Schools (${schools.length})</div>`;
            schools.forEach((f, index) => {
                html += createFacilityCard(f, '#7c3aed', '#ffffff', `school-${index}`);
